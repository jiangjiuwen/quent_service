...
if [ "${OS_NAME}" != "Darwin" ]; then
    echo "prod_deploy_macos.sh 仅支持 macOS" >&2
    exit 1
fi
...
QUANT_WATCH_POLL_SECONDS=${WATCH_POLL_SECONDS}
QUANT_WATCH_DEBOUNCE_SECONDS=${WATCH_DEBOUNCE_SECONDS}
QUANT_WATCH_STATE_FILE="${WATCH_STATE_FILE}"
EOF
}
...
<dict>
    ...
    <array>
        <string>/bin/bash</string>
        <string>-lc</string>
        <string>set -a; source "${PROD_ENV_FILE}"; set +a; export QUANT_WATCH_WORKSPACE_DIR="${WORKSPACE_DIR}"; export QUANT_WATCH_DEPLOY_SCRIPT="${WORKSPACE_DIR}/scripts/prod_deploy.sh"; exec "${PROD_VENV_DIR}/bin/python" "${WORKSPACE_DIR}/scripts/prod_watch.py"</string>
    </array>
    <key>WorkingDirectory</key>
